...
def odeint(func, y0, y11, y_input, y_phy, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    ...
    solver = SOLVERS[method](func=func, y0=y0, y11=y11, y_input=y_input, y_phy=y_phy, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        if y_phy == 1:
            #solution,FF,PHY,PHY_ori = solver.integrate(t)
            solution,PHY = solver.integrate(t)
        elif y_phy == 0:
