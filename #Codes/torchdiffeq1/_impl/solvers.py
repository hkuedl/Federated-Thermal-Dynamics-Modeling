...
class FixedGridODESolver(metaclass=abc.ABCMeta):
    ...
    def __init__(self, func, y0, y11, y_input, y_phy, step_size=None, grid_constructor=None, interp="linear", perturb=False, **unused_kwargs):
    ...
    def integrate(self, t):
        ...
        y0 = self.y0
        # if self.y_phy == 1:
        #     FF = torch.zeros((self.y11.shape[0]-1,self.y11.shape[1],1,1))
        #     PHY = torch.zeros((self.y11.shape[0]-1 , self.y11.shape[1] , 1 , self.y11.shape[-1]+1))
        #     PHY_ori = torch.zeros((self.y11.shape[0]-1 , self.y11.shape[1] , 1 , self.y11.shape[-1]+1))

        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            dt = t1 - t0
            y11_i = self.y11[ii,:,:,:]
            # if self.y_phy == 1:
            #     dy, ff, phy, phy_ori = self._step_func(self.func, t0, dt, t1, y0, y11_i, self.y_input, self.y_phy)
            #     y1 = y0 + dy
            #     FF[ii,:,:,:] = ff
            #     for ij in range(len(phy)):
            #         PHY[ii,:,:,ij:ij+1] = phy[ij]
            #         PHY_ori[ii,:,:,ij:ij+1] = phy_ori[ij]
            if self.y_phy == 1:
                dy, PHY = self._step_func(self.func, t0, dt, t1, y0, y11_i, self.y_input, self.y_phy)
                y1 = y0 + dy

            elif self.y_phy == 0:
                dy = self._step_func(self.func, t0, dt, t1, y0, y11_i, self.y_input, self.y_phy)
                y1 = y0 + dy
            ...
            ii += 1
        if self.y_phy == 1:
            #return solution, FF, PHY, PHY_ori
            return solution, PHY
        elif self.y_phy == 0:
            return solution
